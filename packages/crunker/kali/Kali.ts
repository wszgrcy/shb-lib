...
class Kali {
  ...
  private tempo_best_overlap_position(
    ...
    const f: Float32Array = t.overlap_buf;

    let j: size_t;
    let best_pos: size_t;

    // NOTE: changed to zero-fill shift
    let prev_best_pos: size_t = (t.search + 1) >>> 1;
    let step: size_t = 64;
    let i: size_t = (best_pos = t.quick_search ? prev_best_pos : 0);

    let diff: float;
    let least_diff: float = this.difference(
  ...
  private tempo_overlap(
    ...
    let j: size_t = 0;
    let k: size_t = 0;
    const fade_step: float = 1.0 / t.overlap;

    for (i = 0; i < t.overlap; i++) {
      const fade_in: float = fade_step * i;
      const fade_out: float = 1.0 - fade_in;
  ...
  public output(samples: Float32Array): size_t {
    const t = this.t;
    const n = Math.min(samples.length, t.output_fifo.occupancy());
    t.samples_out += n;
  ...
  public flush(): void {
    const t = this.t;
    const samples_out: uint64_t = handleInt(t.samples_in / t.factor + 0.5);
    const remaining: size_t =
  ...
  public setup(
    ...
    segment_ms: double | null = null,
    search_ms: double | null = null,
    overlap_ms: double | null = null,
  ): void {
    const profile = 1;
    const t = this.t;
    ...
    if (!t.is_initialized) {
      ...
      const new_overlap = new Float32Array(t.overlap * t.channels);
      let start = 0;
