...
export class Crunker {
   ...
   * @internal
   */
  private _createContext(sampleRate: number = 44_100) {
    return {
      createBuffer: this.createBuffer,
      decodeAudioData: this.decodeAudioData,
      sampleRate,
    };
  }
  ...
  private decodeAudioData(buffer: ArrayBuffer | ArrayLike<number>) {
  ...
  async decodeFiles(buffers: Buffer[]) {
    return Promise.all(
      buffers.map((buffer) =>
        this.context.decodeAudioData(new Uint8Array(buffer.buffer)),
      ),
    );
  }
}
